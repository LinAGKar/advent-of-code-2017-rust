...
fn main() {
    ...
    println!("{}", Generator { num: start_a, factor: 16807 }.filter(|x| x % 4 == 0).zip(Generator { num: start_b, factor: 48271 }.filter(|x| x % 8 == 0)).take(5000000).filter(|&(a, b)| a & 0xFFFF == b & 0xFFFF).count());
}
